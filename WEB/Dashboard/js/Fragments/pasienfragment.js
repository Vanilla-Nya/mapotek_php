    // Enhanced Pasien Fragment with Patient History Modal
    class PasienFragment {
        ...
        // ✅ FIXED: Query pemeriksaan directly by finding antrian with same encounter
        async loadPatientHistory(patientId) {
            ...
            try {
                ...
                const historyPromises = uniqueAntrianData.map(async (displayAntrian) => {
                    ...
                    // Step 2: Get ALL antrian entries with this encounter (to find the one with pemeriksaan)
                    ...
                    if (allAntrianResult.success && allAntrianResult.data && allAntrianResult.data.length > 0) {
                        // Extract all antrian IDs for this encounter
                    ...
                    if (pemeriksaan) {
                        ...
                        try {
                            ...
                            if (vitalResult.success && vitalResult.data) {
                                ...
                                console.log(`   ✅ Loaded ${vitalSigns.length} vital signs`);
                            }
                        } catch (error) {
                            console.warn(`   ⚠️ Failed to load vital signs:`, error);
                        }
                        
                        // Get diagnosis ICD-X (diseases)
                        try {
                            ...
                            if (diagnosisResult.success && diagnosisResult.data) {
                                ...
                                console.log(`   ✅ Loaded ${diagnosa.length} ICD-X diagnoses`);
                            }
                        } catch (error) {
                            console.warn(`   ⚠️ Failed to load ICD-X:`, error);
                        }
                        
                        // Get diagnosis ICD-IX (procedures)
                        try {
                            ...
                            if (prosedurResult.success && prosedurResult.data) {
                                ...
                                console.log(`   ✅ Loaded ${diagnosaProsedur.length} ICD-IX procedures`);
                            }
                        } catch (error) {
                            console.warn(`   ⚠️ Failed to load ICD-IX:`, error);
                        }
                        
                        // Get obat (medicines given)
                        try {
                            ...
                            if (obatResult.success && obatResult.data) {
                                ...
                                console.log(`   ✅ Loaded ${obat.length} medicines`);
                            }
                        } catch (error) {
                            console.warn(`   ⚠️ Failed to load medicines:`, error);
                        }
                        
                        // Get resep with details
                        try {
                            ...
                            if (resepResult.success && resepResult.data && resepResult.data.length > 0) {
                                ...
                                console.log(`   ✅ Loaded ${resep.length} prescriptions`);
                            }
                        } catch (error) {
                            console.warn(`   ⚠️ Failed to load prescriptions:`, error);
                        }
                    } else {
                        console.warn(`   ⚠️ No pemeriksaan found for encounter ${encounterId}`);
                    }
        ...
        // ✅ UPDATED: renderPemeriksaanDetails now includes ICD-IX procedures and medicines given
        renderPemeriksaanDetails(pemeriksaan, anamnesa, diagnosa, diagnosaProsedur, obat, resep) {
            let html = '';
            
            // Anamnesa Section... (existing code)
            
            // Diagnosis ICD-X Section... (existing code)
            
            // ✅ NEW: Diagnosis ICD-IX (Procedures) Section
                ...
                obat.forEach(o => {
                    html += `
                        <tr>
                            ...
                            <td><span class="badge bg-info">${this.escapeHtml(o.obat?.jenis_obat || '-')}</span></td>
        ...
        renderHistoryItem(history, index) {
            const { antrian, pemeriksaan, anamnesa, diagnosa, resep, doctor } = history;
                ...
                <div class="accordion-item border mb-2">
                    ...
                    <div id="history${index}" class="accordion-collapse collapse ${index === 0 ? 'show' : ''}" 
                        ...
                        <div class="accordion-body">
                            ${pemeriksaan ? this.renderPemeriksaanDetails(pemeriksaan, anamnesa, diagnosa, resep) : `
        ...
        renderPemeriksaanDetails(pemeriksaan, anamnesa, diagnosa, resep) {
            let html = '';
            ...
            // If no data
            if (!anamnesa && (!diagnosa || diagnosa.length === 0) && (!resep || resep.length === 0)) {
                html += `
                    <div class="text-center text-muted py-3">
                        <i class="bi bi-info-circle me-2"></i>
                        <small>Belum ada detail pemeriksaan</small>
                    </div>
                `;
            }
